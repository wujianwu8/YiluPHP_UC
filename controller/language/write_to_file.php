...
if (!logic_permission::I()->check_permission('user_center:write_lang_to_php_file')) {
...
if (empty($project_info['file_dir'])){
    unset($params,$project_info);
    return code(4,'PHP语言包目录设置不正确');
}

//读取语言包文件
if (!is_dir($project_info['file_dir'])){
    unset($params,$project_info);
    return code(5,'PHP语言包目录不存在');
}
...
foreach ($project_info['language_types'] as $lang){
    if(!$langfile = fopen($project_info['file_dir'].$lang.'.php', "w")){
        return json(10,'打开文件失败：'.$project_info['file_dir'].$lang.'.php');
    }
    $txt = "<?php
/**
