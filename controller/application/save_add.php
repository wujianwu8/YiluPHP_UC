...
$params['ctime'] = time();
if(false === $app->model_application->insert_table($params)){
    unset($params);
    return_code(1, '保存失败');
}

$permission_keys = $app->model_permission->permission_control_keys();
foreach ($permission_keys as $permission_key => $name){
    $app->model_permission->insert_table([
        'permission_name' => $name,
        'permission_key' => $permission_key,
        'app_id' => $params['app_id'],
        'is_fixed' => 1,
    ]);
    if($permission = $app->model_permission->find_table(['app_id' => $params['app_id'],'permission_key' => $permission_key], 'permission_id')){
