 ...
 * @group 内部接口
 * @name 撤消给用户的一个权限
 * @desc 在调用接口前先判断授权人是否有权删除他人的权限
 * @method GET|POST
 * @uri /internal/revoke_permission
...
if (false === model_user_permission::I()->delete(['uid'=>$params['uid'], 'permission_id'=>$permission_info['permission_id']])){
    unset($params);
    return code(1,'撤消授权失败');
}
