...
if(!$cache_code = redis_y::I()->get($code_cache_key)){
    ...
    return code(2,'请先发送手机验证码');
}
if ($cache_code!=$params['verify_code']){
    unset($code_cache_key, $params, $complete_phone, $cache_code);
    return code(3, YiluPHP::I()->lang('verify_code_error'));
}
...
//如果没有昵称，则生成一个昵称
if(!$nickname){
    $nickname = model_user::I()->mobile_to_nickname($params['mobile']);
}
//保证昵称的唯一性
...
if($is_bind===1){
    model_user_identity::I()->insert_table($identity_plat);
    //把登录身份存入缓存
    model_user_identity::I()->cache_user_identity($identity_plat['type'], $identity_plat['identity'], $identity_plat['uid']);
}
...
//返回结果
return json(0, YiluPHP::I()->lang('sign_up_successful'), ['tlt' => $tlt]);
