...
unset($params, $user_info);
if(!$identity = $app->model_user_identity->find_table(['uid'=>$self_info['uid'], 'type'=>'QQ'], 'identity', $self_info['uid'])){
    unset($identity);
    return_code(4,'您未绑定QQ账号');
}

if (!$app->model_user_identity->delete_identity('QQ', $identity['identity'], $self_info['uid'])){
    unset($identity);
    return_code(1,'解绑失败');
}
unset($identity);
