...
unset($params, $user_info);
if(!$identity = model_user_identity::I()->find_table(['uid'=>$self_info['uid'], 'type'=>'ALIPAY'], 'identity', $self_info['uid'])){
    unset($identity);
    return code(4,'您未绑定支付宝账号');
}

if (!model_user_identity::I()->delete_identity('ALIPAY', $identity['identity'], $self_info['uid'])){
    unset($identity);
    return code(1,'解绑失败');
}
unset($identity);
