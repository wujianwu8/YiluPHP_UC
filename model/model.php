...
class model
{
     ...
     * @return array 数据列表
     */
    function count($where, $field_value=null)
    {
...
//  	PDO::FETCH_LAZY            创建关联数组和索引数组，以及包含列属性的一个对象，从而可以在这三种接口中任选一种。
     ...
     * @return array 数据列表
     */
    function paging_select(array $where, int $page, int $page_size, string $order_by='', string $fields='*', string $field_value=null)
    {
        $table_name = $this->sub_table($field_value);
        $connection = $this->sub_connection($field_value);
        ...
        $where && $sql .= ' WHERE ';
        $sql .= implode(' AND ', $arr) . (trim($order_by)!==''?' ORDER BY '.$order_by : '') .' LIMIT :start, :page_size ';
        $page = intval($page);
...
//  	PDO::FETCH_LAZY            创建关联数组和索引数组，以及包含列属性的一个对象，从而可以在这三种接口中任选一种。
     ...
     * @return array 数据列表
     */
    function select_all($where, $order_by='', $fields='*', $field_value=null)
    {
        $table_name = $this->sub_table($field_value);
        $connection = $this->sub_connection($field_value);
        ...
        $where && $sql .= ' WHERE ';
        $sql .= implode(' AND ', $arr) . (trim($order_by)!==''?' ORDER BY '.$order_by : '');
        try {
            $stmt = $GLOBALS['app']->mysql($connection)->prepare($sql);
            foreach ($where as $key => &$value) {
                $direct_assign = true;
...
//  	PDO::FETCH_LAZY            创建关联数组和索引数组，以及包含列属性的一个对象，从而可以在这三种接口中任选一种。
     ...
     */
    function find_table($where, $fields='*', $field_value=null)
    {
        $table_name = $this->sub_table($field_value);
        $connection = $this->sub_connection($field_value);
...
//  	PDO::FETCH_LAZY            创建关联数组和索引数组，以及包含列属性的一个对象，从而可以在这三种接口中任选一种。
     ...
     * @return boolean
     */
    function update_table(&$where, &$data)
    {
        ...
        foreach($tables as $item) {
            $sql = 'UPDATE ' . $item['table_name'] . ' SET ' . implode(',', $set) . ' WHERE ' . implode(' AND ', $where_sql);
            try {
                $stmt = $GLOBALS['app']->mysql($item['connection'])->prepare($sql);
                foreach ($data as $key => $value) {
                    $stmt->bindValue(':' . $key, $value);
    ...
    function delete($where, $data=[])
    {
        ...
        $count = 0;
        foreach($tables as $item) {
            $sql = 'DELETE FROM ' . $item['table_name'];
            if (!empty($where)){
